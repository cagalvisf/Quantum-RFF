...
class LP_approx_GPR():
    ...
    def Phi_matrix(self, X):
        ...
        return (np.reshape(np.apply_along_axis(phi_vector, 1, X), (len(X) , 2*len(samples)))).astype('float64')
    ...
    def posterior(self, Xp):
        ...
        phisf = self.Phi_matrix(Xp).T
        Phif = self.Phi_matrix(x_train)
        
        A = (alpha * Phif.T @ Phif + (jitter + M * sigma2) * np.identity(2 * M)) / alpha

        print("A: ", A.shape)
        print("Phisf: ", phisf.shape)
        print("Phif: ", Phif.shape)
        A_1 = np.linalg.inv(A)
        mu = phisf.T @ A_1 @ Phif.T @ y_train
